 ...
 * @returns Accessor of the resolved value
 *
 * @example
 * ```tsx
 ...
 * @param defaultValue - Default value if source is undefined or path not found
 * @returns Accessor of the resolved string
 */
 ...
 * @returns Accessor of the data model for this surface
 */
export function useDataModel(surfaceId: string): Accessor<DataModel> {
  const { getDataModel } = useSurfaceContext();

  return createMemo(() => getDataModel(surfaceId));
}

/**
 * Hook for two-way data binding in form components.
 * Returns both the current value accessor and a setter function.
 *
 * @param surfaceId - The surface ID
 * @param source - The dynamic value (must be a path binding for setting)
 * @param defaultValue - Default value if not found
 * @returns Tuple of [Accessor<value>, setValue]
 *
 * @example
 * ```tsx
