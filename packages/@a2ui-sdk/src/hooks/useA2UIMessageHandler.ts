...
export function useA2UIMessageHandler(): A2UIMessageHandler {
  ...
  let messageBuffer = new Map<string, A2UIMessage[]>();

  // Track created surfaces synchronously (state updates are async)
  let createdSurfaces = new Set<string>();

  /**
   * Applies buffered messages for a surface.
   */
  const applyBufferedMessages = (surfaceId: string) => {
    const buffered = messageBuffer.get(surfaceId);
    ...
    messageBuffer.delete(surfaceId);
  };

  /**
   * Buffers a message for later processing.
   */
  const bufferMessage = (surfaceId: string, message: A2UIMessage) => {
    const existing = messageBuffer.get(surfaceId) ?? [];
    messageBuffer.set(surfaceId, [...existing, message]);
  };

  const processMessage = (message: A2UIMessage) => {
